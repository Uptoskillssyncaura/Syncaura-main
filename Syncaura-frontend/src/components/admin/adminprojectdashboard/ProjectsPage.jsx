  ...
  return (
    <div className="relative w-36 h-36 shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
        ...
        <span className="text-[12px] font-semibold text-gray-400 dark:text-gray-500 mt-[3px]">{t("admin_healthy")}</span>
      </div>
    </div>
  );
}
...
───────────────────────────────────────────────────────────── */
const getProjects = (t) => [
  { id: 1, name: 'Website Redesign', sub: 'Marketing Q3 Priority', av: 'S', avBg: '#ef4444', owner: 'Sarah J.', prog: 50, progColor: '#3b82f6', status: t("admin_active"), sBg: '#bfdbfe', sColor: '#1d4ed8', health: '#22c55e', deadline: 'Oct 24, 2023', risk: true },
  { id: 2, name: 'Mobile App Migration', sub: 'Engineering', av: 'M', avBg: '#3b82f6', owner: 'Mike T.', prog: 25, progColor: '#f97316', status: t("admin_at_risk"), sBg: '#fee2e2', sColor: '#dc2626', health: '#fca5a5', deadline: 'Nov 12, 2023', risk: false },
  { id: 3, name: 'Q4 Marketing Campaign', sub: t("admin_marketing"), av: 'E', avBg: '#f97316', owner: 'Emily R.', prog: 75, progColor: '#3b82f6', status: t("admin_delayed"), sBg: '#fef9c3', sColor: '#92400e', health: '#fbbf24', deadline: 'Oct 10, 2023', risk: false },
  { id: 4, name: 'Design System V2', sub: 'Design Internal', av: 'D', avBg: '#8b5cf6', owner: 'David L.', prog: 90, progColor: '#22c55e', status: t("admin_review"), sBg: '#dcfce7', sColor: '#16a34a', health: '#22c55e', deadline: 'Sep 30, 2023', risk: true },
  { id: 5, name: 'API Integration', sub: t("admin_engineering"), av: 'J', avBg: '#22c55e', owner: 'James K.', prog: 100, progColor: '#22c55e', status: t("admin_completed"), sBg: '#f3f4f6', sColor: '#6b7280', health: '#22c55e', deadline: 'Sep 15, 2023', risk: true },
]

const getResources = (t) => [
  { label: t("admin_engineering"), pct: 85, color: '#3b82f6', dark: '#00d4ff' }, // Cyan
  { label: t("admin_design"), pct: 92, color: '#a855f7', dark: '#bc13fe' },      // Purple
  { label: t("admin_marketing"), pct: 45, color: '#ef4444', dark: '#ff00ff' },    // Pink/Magenta
  { label: t("admin_product_management"), pct: 60, color: '#f97316', dark: '#ff6b00' }, // Orange
]
...
export default function ProjectsPage() {
  ...
  const filtered = getProjects(t).filter(p =>
    ...
    <div className="bg-white dark:bg-[#08090ac7] min-h-screen  font-sans transition-colors duration-300">
      <div className="w-full">
        ...
        <div className="grid grid-cols-5 gap-[10px] mb-[14px]">
          <StatCard label={t("admin_total_projects")}
            value="42"
            highlight={false}
            iconBg="#eff6ff"
            iconColor="#3b82f6"
          ...
          />
          <StatCard label={t("admin_active")}
            value="12"
            highlight={false}
            iconBg="#eff6ff"
            iconColor="#3b82f6"
            Icon={FaCirclePlay} />
          <StatCard label={t("admin_completed")}
            value="24"
            highlight={false}
            iconBg="#dcfce7"
            iconColor="#16a34a"
            ...
            footerColor="#16a34a"
            footerBg="#dcfce7" />
          <StatCard label={t("admin_delayed")}
            value="4"
            borderColor="#d97706"
            highlight={true}
            iconBg="#fef9c3"
            ...
            footerColor="#94a3b8" />
          <StatCard label={t("admin_at_risk")}
            value="2"
            borderColor="#dc2626"
            highlight={true}
            iconBg="#fee2e2"
        ...
        <div className="bg-white dark:bg-[#1c1d1d] border border-gray-200 dark:border-zinc-800 rounded-[10px] px-5 py-4 mb-3 shadow-sm">
          <p className="text-2xl font-bold text-black dark:text-white mb-4 tracking-normal leading-none">{t("admin_project_health_status")}</p>
          <div className="flex items-center gap-11 pl-[6px]">
            <DonutChart />
            <div className="flex gap-10">
              {[['#22c55e', t("admin_on_track"), '(65%)'], ['#f59e0b', t("admin_warning"), '(20%)'], ['#ef4444', t("admin_critical"), '(15%)']].map(([c, lbl, pct]) => (
        ...
        <div className="bg-white dark:bg-[#1c1d1d] border border-gray-200 dark:border-zinc-800 rounded-[10px] px-5 py-4 mb-3 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <p className="text-2xl font-bold text-black dark:text-white tracking-normal leading-none">{t("admin_resource_allocation")}</p>
            <div className="flex bg-gray-50 dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-lg p-0.5">
              {[t("admin_by_team"), t("admin_by_project")].map(t => (
          ...
          {getResources(t).map(r => (
            <div key={r.label} className="mb-[13px]">
              <div className="flex justify-between mb-[5px]">
                ...
                <span className="text-[11px] font-bold text-black dark:text-white">{r.pct}% {t("admin_capacity")}</span>
              </div>
        ...
        <div className="bg-white dark:bg-[#1c1d1d] border border-gray-200 dark:border-zinc-800 rounded-[10px] overflow-hidden shadow-sm">
          <div className="flex items-center justify-between px-[14px] py-3 border-b border-gray-200 dark:border-zinc-800">
            <span className="text-2xl font-bold text-zinc-500 dark:text-zinc-600 tracking-normal leading-none">{t("admin_all_projects")}</span>
            <div className="flex gap-2 items-center">
              <div className="relative flex items-center">
                <span className="absolute left-2 flex pointer-events-none"><IcoSearch /></span>
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t("admin_filter_projects")} className="pl-[26px] pr-[10px] py-[5px] border border-gray-200 dark:border-zinc-800 rounded-[6px] text-[11px] font-normal text-gray-700 dark:text-zinc-300 bg-white dark:bg-[#0c192e] w-[155px] outline-none" />
              </div>
              <button className="px-3 py-[5px] border border-gray-200 dark:border-zinc-800 rounded-[6px] text-[11px] font-medium bg-white dark:bg-zinc-900 text-gray-500 dark:text-zinc-500 btn-hover">{t("admin_filters")}</button>
            </div>
          </div>
          ...
          <div className="px-[14px] py-[9px] text-[10px] font-normal text-gray-500 dark:text-zinc-500 flex items-center bg-white dark:bg-[#1c1d1d] justify-between border-t border-gray-600 ">
            <span>{t("admin_showing_projects")}</span>
            <div className="flex gap-[5px]"><button className="px-[14px] py-[2px] border border-zinc-100 dark:border-zinc-800 rounded-[4px] bg-white dark:bg-zinc-900 text-black dark:text-zinc-400 btn-hover">{t("admin_prev")}</button><button className="px-[14px] py-[2px] border border-zinc-200 dark:border-zinc-800 rounded-[4px] bg-white dark:bg-zinc-900 text-black dark:text-zinc-400 btn-hover">{t("admin_next")}</button></div>
          </div>
        </div>
