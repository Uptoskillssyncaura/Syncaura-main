...
const MeetingCard = memo(function MeetingCard({
  ...
  if (!googleMeetLink) {
    console.error("Google Meet link is missing");
    return;
  }
  ...
  onClick={!isCompleted && !isUpcoming ? handleJoinMeeting : undefined}
  className={`btn-hover px-3 py-1.5 rounded-full flex items-center justify-center text-xs font-semibold shadow-[0_4px_10px_0_rgba(0,0,0,0.25)] transition min-w-[100px] ${
    isCompleted
      ? "bg-red-500 dark:bg-[#1E293B] dark:text-[#94A3B8] text-yellow-300 cursor-not-allowed"
      : isUpcoming
        ? "bg-[#D9D9D9] dark:bg-[#5e5c5c] dark:text-[#73FBFD] text-gray-700 cursor-not-allowed"
